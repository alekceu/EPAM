...
f_show_from_wich_ip_most_request(){
  requests_ip_uniq=$(cat 1_apache_logs | cut -d " " -f 1 | sort | uniq )
  ip_max=0
  count_max=0
  for ip in $requests_ip_uniq
  do
    count=$( cat 1_apache_logs | cut -d " " -f 1 | sort | grep $ip | wc -l)
...
f_show_most_request_page(){
  ...
  awk {'print $7 "\t" $11'} 1_apache_logs | sort | uniq -c | sort -n | tail -n 1
}

f_count_all_uniq_ip_request(){
  echo "How many requests were there from each ip?"
  awk {'print $1'} 1_apache_logs | sort | uniq -c | sort -n
}

f_non-existent_pages(){
  echo "What non-existent pages were clients referred to";
  cat 1_example_log | grep 404 | awk {'print $7 "\t" $11'} | sort | uniq
}
